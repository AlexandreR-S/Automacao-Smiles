///////////////////////////////////////////////////////////////////////////////
//
// This file was automatically generated by RANOREX.
// Your custom recording code should go in this file.
// The designer will only add methods to this file, so your custom code won't be overwritten.
// http://www.ranorex.com
//
///////////////////////////////////////////////////////////////////////////////

using System;
using System.Collections.Generic;
using System.Drawing;
using System.Text;
using System.Text.RegularExpressions;
using System.Threading;
using WinForms = System.Windows.Forms;

using Ranorex;
using Ranorex.Core;
using Ranorex.Core.Repository;
using Ranorex.Core.Testing;

namespace POCSmiles.Mobile.RxBrowser.SelecaoVoos
{
    public partial class SelecionarVoo
    {
        /// <summary>
        /// This method gets called right after the recording has been started.
        /// It can be used to execute recording specific initialization code.
        /// </summary>
        private void Init()
        {
            // Your recording specific initialization code goes here.
        }

        private void SelectFlyPayment(string type, string route){
        	try {
        		if (type.Equals("") || type.Equals("NA") || type.Equals("N/A"))
        		{
        			return;
        		}
        		Report.Info("Selecting payment type '" + type + "' in '" + route + "'");
        		Smiles_UserCode.SelectFlyPaymentMobile(repo,route,type);
        		Report.Info("Payment selected");
        	} catch (Exception e) {
        		Report.Error("SelecionarVoo - Error selecting payment type '" + type + "' in '" + route + "'.Error: " + e.Message);
        		throw new System.Exception("Fatal error");
        	}
        }
        
        private void SelectRatePayment(string type){
        	try {
        		if (type.Equals("") || type.Equals("NA") || type.Equals("N/A"))
        		{
        			return;
        		}
        		Report.Info("Selecting rate payment  '" + type + "'");
        		switch (type)
        		{
        			case "Dinheiro":
        				repo.RxBrowser.Dom.Input_TaxasDinhero.Touch();
        				break;
        			case "Milhas":
        				repo.RxBrowser.Dom.Input_TaxasMilhas.Touch();
        				break;
        		}
        		Report.Info("Rate payment selected");
        	} catch (Exception e) {
        		Report.Error("SelecionarVoo - Error selecting payment type '" + type + "'. Error: " + e.Message);
        		throw new System.Exception("Fatal error");
        	}
        }
        private void WaitLoad(string timeout){
        	try {
        		Report.Info("Wainting system load");
        		Smiles_UserCode.WaitSmilesLoadGIFMobile(repo, int.Parse(timeout));
        		Report.Info("System loaded");
        	} catch (Exception e) {
        		Report.Error("BuscarVoos - Error waiting system load. Timeout:" + timeout + ". Error: " + e.Message);
        		throw new System.Exception("Fatal error");
        	}
        }

        public void WaitForDocumentLoad()
        {
        	try {
        		if (!repo.RxBrowser.Self.Element.GetAttributeValueText("OSVersion").ToUpper().Contains("IOS")) {
        			Report.Log(ReportLevel.Info, "Invoke Action", "Invoking WaitForDocumentLoaded() on item 'RxBrowser.Dom'.", repo.RxBrowser.Dom.SelfInfo, new RecordItemIndex(3));
		            repo.RxBrowser.Dom.Self.WaitForDocumentLoaded();
		            Delay.Milliseconds(0);
        		}
        	} catch (Exception e) {
        		Report.Failure("Error launched trying to wait dom state. Error: " + e.Message);
        		throw new Exception("Fatal error");
        	}
        }

    }
}
